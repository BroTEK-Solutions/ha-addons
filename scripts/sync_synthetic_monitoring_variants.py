...
def component_body(source: Path, component: str) -> bytes:
    """One launcher or UI source as it should appear in a variant App."""
    if source.suffix != ".go":
    ...
    marker = (
        f"// Code generated by scripts/sync_synthetic_monitoring_variants.py from "
        f"synthetic_monitoring_shared/{component}/{source.name}. DO NOT EDIT.\n\n"
    )
    ...
    for component in ("launcher", "ui"):
        for source in sorted((SOURCE / component).iterdir()):
            ...
            if source.name not in {"go.mod", "go.sum"} and source.suffix != ".go":
                continue
            result[ROOT / slug / component / source.name] = component_body(
                source, component
            )
